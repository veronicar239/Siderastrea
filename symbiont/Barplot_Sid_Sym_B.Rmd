...
```{r}
# import data
de.genes <- readRDS("Sid_Symbiont_Breviolum_DE_genes_OC_RC_padj.1.rds")
...
```{r}
# import data
de.genes <- readRDS("Sid_Symbiont_Breviolum_DE_genes_LC_RC_padj.1.rds")
...
```{r}
# import data
de.genes <- readRDS("Sid_Symbiont_Breviolum_DE_genes_LO_RO_padj.1.rds")
...
```{r}
# import data
de.genes <- readRDS("Sid_Symbiont_Breviolum_DE_genes_OO_RO_padj.1.rds")
...
```{r}
# import data
de.genes <- readRDS("Sid_Symbiont_Breviolum_DE_genes_RO_RC_padj.1.rds")
...
```{r}
# import data
de.genes <- readRDS("Sid_Symbiont_Breviolum_DE_genes_OO_RC_padj.1.rds")
...
```{r}
summary2 <- rbind(summary_DEG_2, summary_DEG_3, summary_DEG_5, summary_DEG_6, summary_DEG_9)
...
```{r}
summary2$Destination <- c("Control", "Control", "Control", "Control", "Ojo", "Ojo", "Ojo", "Ojo", "Ojo vs. Control", "Ojo vs. Control")
...
write.csv(file = "DEG_summary_Sid_Symbiont_B.csv", x = summary2)
```
...
plot.combo <- ggplot(summary2, aes(x = group, fill = LFC)) + 
  ...
  scale_y_continuous(limits = c(-400, 6400), breaks = seq(-250, 6250, by = 250)) +
  theme_classic() +
...
```{r}
# save plot
ggsave("Sid_Symbiont_B_DEG_barplot.pdf", plot = plot.combo,  width = 5, height = 4, units = "in", dpi = 1200) #width = 6.5, height = 4.5
```
...
```{r}
summary3 <- rbind(summary_DEG_2, summary_DEG_3, summary_DEG_5, summary_DEG_6, summary_DEG_9, summary_DEG_10)

summary3
```

```{r}
summary3$Destination <- c("Control", "Control", "Control", "Control", "Ojo", "Ojo", "Ojo", "Ojo", "Ojo vs. Control", "Ojo vs. Control", "Low vs. Ambient pH", "Low vs. Ambient pH")
...
plot.combo <- ggplot(summary3, aes(x = group, fill = LFC)) + 
  ...
  ylab("Differentially expressed genes") +
  xlab("Transplantation") + 
  scale_y_continuous(limits = c(-1000, 7000), breaks = seq(-500, 7000, by = 500)) +
  theme_classic() +
...
# save plot
#ggsave("Sid_Symbiont_B_DEG_barplot_4-dest-comparisons.pdf", plot = plot.combo,  width = 5, height = 4, units = "in", dpi = 1200) #width = 6.5, height = 4.5
```
