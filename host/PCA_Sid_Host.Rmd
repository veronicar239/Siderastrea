...
```{r message=FALSE}
# save plot
#pdf("Sid_Host_PCA_total-exp_colony_sample-ID.pdf")
...
Question for Dan:
**Is it worth removing outlier Reef.Control.647.18 (also identified in cluster dendrogram as outlier in DESeq2 analysis, sample R_018_C_Sid_Yr2)? Can search for "Cluster dendrogram - vst Blind = FALSE" in DESeq2_Sid_Host.Rmd to see its outlier position in cluster dendrogram. If yes to remove it, then I need to go back and re-run DESeq2.**


# Total expression - Origin
```{r}
# save plot
#pdf("Sid_Host_PCA_total-exp_Origin.pdf")
...
```{r}
# save plot
#pdf("Sid_Host_PCA_total-exp_Destination.pdf")
...
```{r}
# save plot
#pdf("Sid_Host_PCA_total-exp_Colony.pdf")
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Lagoon-Ojo vs. Reef-Ojo (1286 genes)") +
      theme_cowplot()
...
counts_de <- counts.transformed.vst[,c(2,4,6,9,11,24,26,28,31,33,35,38)]
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Ojo-Ojo vs. Reef-Ojo (1870 genes)") +
      theme_cowplot()
...
# select relevant sample columns for the given pairwise comparison
#colnames(counts.transformed.vst)
counts_de <- counts.transformed.vst[,c(13,15,17,19,21,22,24,26,28,31,33,35,38)]
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Reef-Ojo vs. Reef-Control (1562 genes)") +
      theme_cowplot()
...
# select relevant sample columns for the given pairwise comparison
#colnames(counts.transformed.vst)
counts_de <- counts.transformed.vst[,c(23:38)]
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Lagoon-Control vs. Reef-Control (1273 genes)") +
      theme_cowplot()
...
# select relevant sample columns for the given pairwise comparison
#colnames(counts.transformed.vst)
counts_de <- counts.transformed.vst[,c(1,3,5,7,8,10,23,25,27,29,30,32,34,36,37)]
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Ojo-Control vs. Reef-Control (1455 genes)") +
      theme_cowplot()
...
# select relevant sample columns for the given pairwise comparison
#colnames(counts.transformed.vst)
counts_de <- counts.transformed.vst[,c(12,14,16,18,20,23,25,27,29,30,32,34,36,37)]
...
plot <- DESeq2::plotPCA(counts_de, returnData = TRUE, intgroup = c("group")) %>% 
      ...
      xlab(paste0("PC1: ",percentVar[1],"% variance")) +
      ylab(paste0("PC2: ",percentVar[2],"% variance")) +
      labs(title = "Lagoon-Ojo vs. Lagoon-Control (210 genes)") +
      theme_cowplot()
...
```{r}
# save plot
#pdf("Sid_Host_PCALO_LC_subset.pdf")
